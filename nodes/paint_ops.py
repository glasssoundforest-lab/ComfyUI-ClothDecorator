...
def _safe_float(value: float, default: float = 0.0, lo: float | None = None, hi: float | None = None) -> float:
    """NaN/Inf/変換不能な値を default に丸め、指定があれば [lo, hi] にクランプする。"""
    try:
        v = float(value)
...
def _blend(base: np.ndarray, effect: np.ndarray, mode: BlendMode, opacity: float) -> np.ndarray:
    ...
    opacity = _safe_float(opacity, default=1.0, lo=0.0, hi=1.0)
...
def gradient_fill(
    ...
    angle_deg = _safe_float(angle_deg, default=0.0)
...
def brightness_contrast(
    ...
    """brightness/contrast は -1.0〜1.0。0.0=無変化。範囲外・NaN/Infはクランプ/無効化される。"""
    brightness = _safe_float(brightness, default=0.0, lo=-1.0, hi=1.0)
    contrast = _safe_float(contrast, default=0.0, lo=-1.0, hi=1.0)
...
def hue_shift(image: np.ndarray, degrees: float) -> np.ndarray:
    """HSV色空間で色相を degrees(°) だけ回転させる。NaN/Inf/極端な値は安全に処理される。"""
    from PIL import Image

    degrees = _safe_float(degrees, default=0.0) % 360.0  # 有限な値に丸めた上で周期を1周分に正規化
...
def _parse_color(color: str) -> tuple[int, int, int]:
    ...
    if "," in color:
        ...
        if len(parts) == 3:
            # 範囲外（負数・256以上）の入力もクラッシュさせず 0-255 にクランプする
...
def apply_within_mask(
    ...
    # feather_mask() 自体がNaN/Inf/負値/巨大値を安全に処理するため、無条件に呼んでよい
    m = feather_mask(mask, feather_px)
    opacity = _safe_float(opacity, default=1.0, lo=0.0, hi=1.0)
    m = np.clip(m, 0.0, 1.0) * opacity
