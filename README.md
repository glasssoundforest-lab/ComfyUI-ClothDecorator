...
- **カテゴリ別ブロック整形（`group_by_category`）**: 🧵 Prompt Composer /
  ...
  **注意（誤検出防止の仕様）**: カテゴリ判定・衝突検出は「タグ全体が丸ごと
  ...
  場合（例: `free_text = "red, blue"` や `"黄色, 黒"`）のみ、カテゴリ
  分類・衝突検出の対象になります。日本語の基本色名（黄色・黒など）・
  日本の伝統色（藍色など）・英語色名のいずれも認識されますが、
  対象語（subject）は `nodes/subject_data.py` に登録された語と完全一致
  する場合のみ対象になります。なお「ミニスカート」「マキシスカート」
  「ロングスカート」のような丈違い・スタイル違いの服は、意図的に
  それぞれ別の服として区別しており、互いに衝突として検知されます
  （「スカート」とも衝突します）。
