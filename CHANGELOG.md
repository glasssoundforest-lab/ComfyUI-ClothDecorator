...
## v0.9.4
- **他に見逃されている衝突が無いか体系的に調査し、「ドレス」の丈違いに
  同種のギャップを発見・修正**: 「スカート」は丈違い（ミニ/マキシ/タイト/
  ロング）が揃っているのに対し、「ドレス」はスタイル違い（Aライン・
  マーメイド等18種）は充実していたが、丈違い（ミニドレス/マキシドレス/
  ロングドレス）が1件も登録されていなかった。スカートと同じ方針で
  `subject_data.py` に `ミニドレス`(mini dress)・`マキシドレス`
  (maxi dress)・`ロングドレス`(long dress) を追加し、互いに・
  「ワンピース」とも衝突検知の対象にした
  - 調査の結果、パンツ/コート/ジャケット/シャツは素材・スタイル別の
    区別が中心で、スカートのような「同一軸（丈）の一部だけ抜けている」
    状態ではないことを確認した（現状維持）
- 新規回帰テスト3件を追加。単体テスト232 → **233件**（全通過）

## v0.9.3
- **「ロングスカート」を新規登録し、他の丈違いスカートと同様に衝突検知の
  対象にした**: 検証の過程で、「ミニスカート」「マキシスカート」等は
  既にそれぞれ別の服として区別され互いに衝突検知の対象になっていた一方、
  「ロングスカート」だけが辞書に無く検知対象外（無視される）という
  一貫性の無い状態だったため、ユーザーに確認の上で修正した:
  - `subject_data.py` に `("ロングスカート", "long skirt")` を追加
  - これにより `free_text = "ロングスカート, スカート"` や
    `"ロングスカート, ミニスカート"` も正しく対象語の衝突として
    検出されるようになった
- 回帰テストを更新（丈違いスカート同士が意図的に衝突扱いになることを
  確認するテストに変更）。単体テスト231 → **232件**（全通過）

## v0.9.2
- **一般的な日本語の色名が色として認識されていなかった不具合を修正**:
  `_find_color_matches()` は英語色名（`BASE_COLORS`）と日本の伝統色
  （`TRADITIONAL_COLORS_JA`、例: 藍色・山吹色）のみを対象にしており、
  「黄色」「黒」「赤」のような**一般的な日本語の色名**は色として
  認識されていなかった（伝統色辞書に無いため）。`BASE_COLOR_EN_TO_JA`
  の日本語訳（黄色・黒・赤・青 等）も色として認識するよう修正した。
  これにより `free_text = "黄色, 黒"` のような組み合わせも正しく
  色の衝突として検出されるようになった
  （例: 「黄色のワンピースと黒のロングスカート」を1つの文として書いた
  場合は、既存仕様通り複合フレーズとして衝突検出の対象外のまま。
  タグを分けて `"黄色, ワンピース, 黒, スカート"` のように書けば、
  色・対象語どちらの衝突も正しく検出される。「ロングスカート」等、
  語彙辞書に完全一致しない修飾語付き変化形は現状引き続き未対応）
- 新規回帰テスト5件を追加。単体テスト227 → **231件**（全通過）

## v0.9.1
- **カテゴリ判定・衝突検出の誤検出バグを修正**: `categorize_term()` /
  `detect_tag_conflicts()` が、色や服の単語を**部分文字列として**検出
  していたため、`"yellow embroidery"`（黄色の刺繍）や
  `"red and blue striped skirt"`（赤と青のストライプ柄のスカート）の
  ような正当な複合フレーズが、丸ごと「色」カテゴリに誤分類されたり、
  存在しない「色の競合」として `confirm_continue` ゲートに引っかかり
  **処理が誤ってブロックされる**問題があった:
  - `vocabulary._find_color_matches()` / `_find_subject_matches()` を、
    カンマ区切りの個々のタグ**全体が丸ごと**語彙上の色名/対象語と
    一致する場合のみ検出する方式に変更（部分一致・埋め込み検出をやめた）
  - これにより、`free_text = "red, blue"`（独立した色タグ2つ）は
    引き続き衝突として正しく検出される一方、
    `free_text = "red and blue striped skirt"`（1つの複合フレーズ）は
    衝突として誤検出されなくなった
  - `categorize_term()` も同じ理由で複合フレーズを安易に「色」等へ
    分類しないよう修正（該当なしの場合は "other" になる）
- 新規回帰テスト4件を追加。単体テスト222 → **227件**（全通過）
...
- **`free_text`欄に複数タグをカンマ区切りで書いた場合、重複が統合されない
  ...
  扱われており、`"dress, jacket, dress, Jacket"` のように同じタグを
  重複して（順不同・大文字小文字違いを含め）書いても一本化されなかった。
  `base_prompt`/`negative_extra` と同様にカンマ区切りで個々のタグへ
  分割してから重複除去するよう修正し、`free_text`内の重複タグ・
  他カテゴリ（色/装飾技法/柄/素材）との重複タグが、入力順序に関係なく
  一つに統合されるようにした
  （例: `color="red", free_text="red, sparkly"` → `"red"`は1回のみ出現）
- 新規回帰テスト4件を追加。単体テスト200 → **203件**（全通過）
- **既知の制約**: 今回の修正は「完全に同じ文字列」の重複除去であり、
  異なる単語で同じ意味を表すタグ（例: "dress" と "gown"、日本語の
  「ドレス」と「ワンピース」を free_text に別々に書いた場合）を
  意味的に一本化する機能ではない。日本語の対象語（subject_hint）1件を
  選ぶ場合のみ、`SUBJECT_HINT_JA_TO_EN` 辞書による同義語の英訳統一が働く。
...
- **全語彙辞書の重複を体系的に検査**（キーだけでなく、日本語ラベル・
  ...
  - キーの重複: decoration_preset(202) / pattern(202) / material(202) /
    伝統色(203) / subject_hint(233) すべてで **0件**
  - 日本語ラベルの重複（同一辞書内で別キーが同じ表示名になる問題）:
    decoration_preset / pattern / material すべてで **0件**
  - 英語プロンプト表現の重複: pattern / material / decoration_preset
    （フレーズ列単位・個別フレーズ単位いずれも）で **0件**
  - **伝統色のhexコード重複を5件発見・修正**（藍色/瑠璃色、群青色/桔梗色、
    柳色/山葵色、黄丹/蜜柑色、滅紫/葡萄鼠が同一hexを共有しており、
    Direct Paintモードでは見分けがつかなかった。それぞれ伝統的な色味の
    違いを反映した近似hexに調整し、全203色が一意なhexを持つように修正）
  - decoration_preset/pattern/material 辞書間でのキー名衝突も無いことを確認
  - subject_hintの英訳重複（ドレス/ワンピース→dress 等、4組）は
    複数の日本語表記が同じ英語概念を指す仕様上の意図的な設計であることを確認
- 上記の重複チェックを継続的な回帰テストとして
  `tests/test_vocab_data_modules.py` に8件追加。単体テスト186 → **194件**
